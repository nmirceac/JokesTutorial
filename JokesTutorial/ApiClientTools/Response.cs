...
namespace ApiClientTools
{
    public class Response
    {
        ...
        public static Response processResponse(ApiClientTools.Request request, System.Net.Http.HttpResponseMessage httpResponse)
        {
            ...
            //     var responseType = new { response.success = false, data = new ExpandoObject() };
            // } else {
            //     var responseType = new { response.success = false, data = new ExpandoObject(), ack = false };
            // }
            
            
            // var failedResponseType = new { response.success = false, data = new ExpandoObject(), message="" };
            ...
            if(response.success)
            {
                ...
                throw new ArgumentException(response.jsonResponse.message, response.jsonResponse.message);
                //throw new ArgumentException(response.jsonResponse.message, response.jsonResponse);
            }
